...
/*
   TODO
      ...
      - Move regex test for valid file inside a fn
      - Add repl
...
module.exports = function(stylus, compile){
  ...
  if (!styleConfig.files.length && styleConfig.src) {
     styleConfig.files   = fs.readdirSync(styleConfig.src).filter(function(file){
          return !file.match(styleConfig.exclude) && file.match(styleConfig.include);
     });
      ...
      // directory
        ...
        fs.readdir(file, function(err, files){
          ...
          files.filter(function(path){
            return !basename(path).match(styleConfig.exclude) && asename(path).match(styleConfig.include);
          }).map(function(path){
            return file + '/' + path;
          }).forEach(compileFile);
        });
  ...
  function watch(file, importedInFiles) {
    ...
    function watchEvent(curr, prev){
       ...
       if ((curr == "force"||curr.mtime > prev.mtime) && Date.now() > (fileObj.timestamp + 20)) {
         if (!basename(file).match(styleConfig.exclude) && basename(file).match(styleConfig.include)) {
            fileObj.counter++;
            compileFile(file);            
  ...
  if (styleConfig.watch){
     watchLib.createMonitor(styleConfig.src, function (monitor) {
         ...
         monitor.on("changed", function (f, curr, prev) {
            if (curr.mtime > prev.mtime && (!basename(f).match(styleConfig.exclude) && basename(f).match(styleConfig.include)) ) {
