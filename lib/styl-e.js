...
module.exports = function(stylus, compile){
  ...
  styleConfig.include     = /\.styl$|\.css$/i;
    ...
    fs.lstat(file, function(err, stat){
      ...
      if (stat.isFile()) {
        fs.readFile(file, 'utf8', function(err, str){
          ...
          options._imports = [];
          compile(str, options, function(err, css){
            if (!css){
              throw new Error('Compile functions doesnt return css');
            }
            ...
            if (err) {
              ...
              if (responsiveSplitter.fileArray.length > 1) {
                 ...
                 //?TODO, move splitter to compile fn?
     ...
     watchLib.createMonitor(styleConfig.src, function (monitor) {
         monitor.on("created", function (f, stat) {
           fs.realpath(f, function(err, path){
              if (err) return;
              // TODO, ADD EXCLUDE; INCLUDE
              if(path && isValidBase(path)) {
         ...
         monitor.on("changed", function (f, curr, prev) {
            if (curr.mtime > prev.mtime && isValidBase(f)  ) {
...
function isValidBase(file) {
   if ((!file.match(styleConfig.exclude) && file.match(styleConfig.include)) && !basename(file).match(styleConfig.exclude) && basename(file).match(styleConfig.include)) {
      return true
   }
   return false;
}
